...
def _apply_op(
    ...
    # elif op_name == "TranslateXYs":
    #     img = F.affine(
    #         img,
    #         angle=0.0,
    #         translate=[int(magnitude*img.size[1]/2),int(magnitude*img.size[0]/2)],
    #         scale=1.0,
    #         interpolation=interpolation,
    #         shear=[0.0, 0.0],
    #         fill=fill,
    #     )
    # elif op_name == "TranslateXYa":
    #     img = F.affine(
    #         img,
    #         angle=0.0,
    #         translate=[int(magnitude*img.size[1]/2),-int(magnitude*img.size[0]/2)],
    ...
    # elif op_name == "ScaleUp":
    #     img = F.affine(
    #         img,
    #         angle=0.0,
    #         translate=[0, 0],
    #         scale= 1 + 2/3*magnitude,
    #         interpolation=interpolation,
    #         shear=[0.0, 0.0],
    #         fill=fill,
    #     )
    # elif op_name == "ScaleDown":
    #     img = F.affine(
    #         img,
    #         angle=0.0,
    #         translate=[0, 0],
    ...
    elif op_name == "RotatePlus":
        img = F.rotate(img, magnitude*60, interpolation=interpolation, fill=fill)
    elif op_name == "RotateMinus":
        img = F.rotate(img, -magnitude*60, interpolation=interpolation, fill=fill)
    ...
    elif op_name == "SolarizePlus":
        img = F.solarize(img,int(255*(1-magnitude/2)))
    elif op_name == "SolarizeMinus":
        img = F.invert(F.solarize(F.invert(img),int(255*(1-magnitude/2))))
    elif op_name == "Posterize":
        img = F.posterize(img, int(round(8*(1-magnitude*1/2))))
    elif op_name == "AutoContrast":
        img = F.autocontrast(img)#Image.blend(img,F.autocontrast(img),magnitude)
    elif op_name == "Equalize":
        img = F.equalize(img)#Image.blend(img,F.equalize(img),magnitude)
    elif op_name == "CutOut":
...
class SingleAugment(torch.nn.Module):
    ...
    def _augmentation_space(self) -> Dict[str, Tuple[Tensor, bool]]:
        ...
        return {
            ...
            # "TranslateXYs": (gamma, True),
            # "TranslateXYa": (gamma, True),
            "ShearX": (gamma, True),
            "ShearY": (gamma, True),
            # "Scale": (gamma, True),
            "ScaleUp": (gamma, False),
            "ScaleDown": (gamma, False),
            "RotatePlus": (gamma, False),
            "RotateMinus": (gamma, False),
            "Hue": (gamma, True),
            "Brightness": (gamma, True),
            "Sharpness": (gamma, True),
            "Contrast": (gamma, True),
            "Saturation": (gamma, True),
            "SolarizePlus": (gamma,False),
            # "SolarizeMinus": (gamma,False),
...
class InformedAugment(torch.nn.Module):
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        index = [random.randint(0, op_len-1) for p in range(0, Naugs)] 
          
        # random_vec = np.random.normal(0,1,Naugs+2)
        # Nplus2_disc = random_vec/np.sqrt(np.sum(random_vec**2))
        # Nball = np.abs(Nplus2_disc[:-2])
        ...
        for n in range(len(index)):
            ...
            # magnitude = random.triangular(0, gamma[index[n]]/3,  gamma[index[n]])
            magnitude = random.uniform(0,  gamma[index[n]])
            # magnitude = Nball[n] * gamma[index[n]]
...
class InformedAugment_GeoApp(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.Naugs = Naugs
        self.Naugs_geo = Naugs_geo
        self.Naugs_app = Naugs_app
        self.gamma = gamma
        self.interpolation = interpolation
        self.fill = fill
    ...
    def _geo_augmentation_space(self) -> Dict[str, Tuple[Tensor, bool]]:
        # gamma = self.gamma
        # num_bins = self.num_magnitude_bins
        return {
            # op_name: (signed)
    ...
    def _app_augmentation_space(self) -> Dict[str, Tuple[Tensor, bool]]:
        # gamma = self.gamma
        # num_bins = self.num_magnitude_bins
        return {
            # op_name: (signed)
    ...
    def forward(self, img: Tensor) -> Tensor:
        ...
        Returns:
            PIL Image or Tensor: Transformed image.
        """
        fill = self.fill   # self.fill
        gamma = self.gamma
        Naugs = self.Naugs
        Naugs_geo = self.Naugs_geo
        Naugs_app = self.Naugs_app
        op_len = len(self._augmentation_space())
        op_geo_len = len(self._geo_augmentation_space())
        ...
        # comb_index = [random.randint(0, op_len-1) for p in range(0, Naugs)]
        # geo_index = [random.randint(0, op_geo_len-1) for p in range(0, Naugs_geo)] 
        # app_index = [random.randint(op_geo_len, len(gamma)-1) for p in range(0, Naugs_app)]      
        
        comb_index = random.sample(range(op_len), Naugs)
        geo_index = random.sample(range(op_geo_len), Naugs_geo)
        app_index = random.sample(range(op_geo_len,len(gamma)),Naugs_app)
        
        index = comb_index + geo_index + app_index
        ...
        for n in range(len(index)):
            ...
            signed = op_meta[op_name]
            magnitude = random.uniform(0,  gamma[index[n]])
            # magnitude = random.uniform(gamma[index[n]]/2,  gamma[index[n]])
